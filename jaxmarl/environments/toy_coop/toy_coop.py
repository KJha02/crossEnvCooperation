...
class ToyCoop(MultiAgentEnv):
    ...
    def __init__(self, max_steps: int = 100, random_reset: bool = False, debug: bool = False, check_held_out: bool = False):
    ...
    def reset(self, key: chex.PRNGKey, params={'random_reset_fn': 'reset_all'}) -> Tuple[Dict[str, chex.Array], State]:
        ...
        def check_match(state):
            ...
            temp = jnp.stack([some_agent_match, some_goal_match], axis=0)  # 2 x num_held_out
    ...
    def custom_reset_fn(self, key, random_reset=False, debug=False):
        ...
        goal_default = jnp.where(debug, og_locations[:2], og_locations[2:])
        goal_pos = jnp.where(random_reset, rand_goal_pos, goal_default)
